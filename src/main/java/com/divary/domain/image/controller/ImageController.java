...
public class ImageController {
    ...
    @Operation(summary = "유저 이미지 업로드", description = "유저 전용 이미지를 업로드합니다. 경로가 자동으로 생성됩니다.")
    @ApiErrorExamples({
            ErrorCode.VALIDATION_ERROR,
            ErrorCode.INTERNAL_SERVER_ERROR
    })
    @PostMapping(value = "/upload/user", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ApiResponse<ImageResponse> uploadUserImage(
            @Parameter(description = "업로드할 이미지 파일", required = true)
            @RequestPart("file") MultipartFile file,
            ...
            @Parameter(description = "추가 경로 (선택사항)", example = "2024/01/15")
            @RequestParam(value = "additionalPath", required = false) String additionalPath,
            
            @Parameter(description = "원본 파일명 (선택사항)", example = "photo.jpg")
            @RequestParam(value = "originalFilename", required = false) String originalFilename
    ) {
        // 자동 경로 생성
        String uploadPath = imageService.generateUserUploadPath(imageType, userId, additionalPath);
        ...
        ImageResponse response = imageService.uploadImage(request);
        return ApiResponse.success("유저 이미지 업로드가 완료되었습니다.", response);
    }

    @Operation(summary = "시스템 이미지 업로드", description = "시스템 전용 이미지를 업로드합니다. 경로가 자동으로 생성됩니다.")
    @ApiErrorExamples({
            ErrorCode.VALIDATION_ERROR,
            ErrorCode.INTERNAL_SERVER_ERROR
    })
    ...
    public ApiResponse<ImageResponse> uploadSystemImage(
            ...
            @Parameter(description = "시스템 이미지 타입", required = true, example = "SYSTEM_DOGAM")
            @RequestParam("imageType") ImageType imageType,
            
            @Parameter(description = "추가 경로 (선택사항)", example = "fish/saltwater")
            @RequestParam(value = "additionalPath", required = false) String additionalPath,
            
            @Parameter(description = "원본 파일명 (선택사항)", example = "fish.jpg")
            @RequestParam(value = "originalFilename", required = false) String originalFilename
    ) {
        // 자동 경로 생성
        String uploadPath = imageService.generateSystemUploadPath(imageType, additionalPath);
        ...
        ImageResponse response = imageService.uploadImage(request);
        return ApiResponse.success("시스템 이미지 업로드가 완료되었습니다.", response);
    }

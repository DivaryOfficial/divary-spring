...
public class DiaryUpdateRequest {
    ...
    @Schema(description = "새로 추가할 이미지 URL 목록")
    private final List<String> newImageUrls;

    // TODO: 프론트에서 스펙 전달해주면 필드 추가 예정 (ex. 폰트, 크기, 손글씨 위치 등)

    @Builder
    public DiaryUpdateRequest(String content, List<Long> deleteImageIds, List<String> newImageUrls) {
        this.content = content;
        this.deleteImageIds = deleteImageIds == null ? null : List.copyOf(deleteImageIds);
        this.newImageUrls = newImageUrls == null ? null : List.copyOf(newImageUrls);
    }
}
